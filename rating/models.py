...
class Debt_service_coverage_ratio(models.Model):
    ...
    def __str__(self):
        return f"{self.sector} - Debt Service Coverage Ratio"
...
class Financial_stmt_quality_Rating(models.Model):
    ...
    audit_type = models.CharField(choices=AUDIT_TYPE_CHOICES, max_length=50)
    audit_discrepancy = models.CharField(choices=AUDIT_DISCREPANCY_CHOICES, max_length=50)

    def calculate_score(self):
        score = 0
        
        if self.customer.total_loan_exposure() > 1000000:
...
class CreditRating(models.Model):
    ...
    financial_stmt_quality = models.ForeignKey(Financial_stmt_quality_Rating, on_delete=models.CASCADE)

    financial_risk = models.PositiveIntegerField(default=0)  # Total financial risk score
    ...
    def calculate_total_financial_risk(self):
        ...
        stmt_quality_score = self.financial_stmt_quality.calculate_score()
